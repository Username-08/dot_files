function fish_prompt --description 'Write out the prompt'
    ...
    # set_color -b black
    printf '%s%s%s%s%s%s%s%s%s%s%s%s%s' (set_color -o $prompt_color) '╭' (set_color brown) $git_info (set_color green) $USER @ $hostname (set_color white) ':' (set_color blue) (prompt_pwd) (set_color cyan) $nix (set_color $prompt_color)
